...
export default function App() {
  ...
  function beginCurrentNote() {
    ...
    noteTimerRef.current = setInterval(() => {
      ...
      for (const targetNote of getNotesAtSameStart(activeNote)) {
        const progress = elapsed / scaleDurationForPractice(targetNote, targetNote.durationMs);
  ...
  function getEventDurationMs(note: NonNullable<typeof currentNote>) {
    ...
    if (nextNote && nextNote.startMs > note.startMs + SAME_BEAT_MS) {
      return Math.max(80, scaleDurationForPractice(note, nextNote.startMs - note.startMs));
    }

    return Math.min(
      ...groupNotes.map((groupNote) => scaleDurationForPractice(groupNote, groupNote.durationMs))
    );
  }
  function findNextEventIndex(note: NonNullable<typeof currentNote>) {
    return notes.findIndex(
      (candidate) => candidate.startMs > note.startMs + SAME_BEAT_MS
    );
  }
  function getDelayToNextEvent(note: NonNullable<typeof currentNote>, nextIndex: number) {
    ...
    return Math.max(
      0,
      scaleDurationForPractice(note, nextNote.startMs - note.startMs) - getEventDurationMs(note)
    );
  }
  ...
  function passCurrentNote() {
    ...
    if (nextIndex >= notes.length || shouldFinishFocusPractice(nextIndex)) {
      ...
      setIsListening(false);
      setCanResumeFromMistake(false);
      setAnalysisStatus(isFocusedPractice ? "\uC9D1\uC911 \uC5F0\uC2B5 \uC644\uB8CC" : "\uC644\uB8CC");
  ...
  function failCurrentNote() {
    ...
    if (nextIndex >= notes.length || shouldFinishFocusPractice(nextIndex)) {
      ...
      setCanResumeFromMistake(false);
      setAnalysisStatus(
        isFocusedPractice ? "\uC9D1\uC911 \uC5F0\uC2B5 \uC644\uB8CC" : "\uC2E4\uC218 \uD3EC\uD568 \uC644\uB8CC"
      );
      void saveCurrentPracticeSession();
      return;
    }
  ...
  function togglePracticeHighlights() {
    ...
    setTimeout(() => applyWeakScoreHighlights(true), 0);
  }
  ...
  function applyLatestMistakeHighlights(force = false) {
    if (!force && !showPracticeHighlights) return;
    if (focusPracticeRange || focusPracticeMeasure !== null) return;
    const weakNoteIndices = new Set(
      focusRanges.flatMap((range) => range.highlightNoteIndices ?? [])
    );
    for (const index of latestMistakeNoteIndices) {
      if (weakNoteIndices.size && !weakNoteIndices.has(index)) continue;
  ...
  function getNotesAtSameStart(note: NonNullable<typeof currentNote>) {
    const sameStartNotes = notes.filter(
      (candidate) => Math.abs(candidate.startMs - note.startMs) < SAME_BEAT_MS
    );
    if (note.isRest) {
  ...
  function getNoteBasedFocusInsights(nextSongId: string, noteSource: PracticeNote[]) {
    const playableNotes = noteSource.filter((note) => !note.isRest);
